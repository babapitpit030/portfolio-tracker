...
class PortfolioController:
    ...
    def add_asset(self) -> None:
        ...
        try:
            ticker = input("Enter asset ticker (e.g., AAPL): ").strip().upper()
            sector = input("Enter sector: ").strip()
            asset_class = input("Enter asset class: ").strip()
            ...
            if current_price:
                ...
                self.cli_view.display_message(f"Current price for {ticker}: ${current_price:.2f}")
            else:
                self.cli_view.display_message(f"Could not fetch current price for {ticker}. Using purchase price.")
